    ...
    <body>
        ...
        @if(\Osiset\ShopifyApp\Util::getShopifyConfig('appbridge_enabled') && \Osiset\ShopifyApp\Util::useNativeAppBridge())
            <script src="{{config('shopify-app.appbridge_cdn_url') ?? 'https://unpkg.com'}}/@shopify/app-bridge{{ \Osiset\ShopifyApp\Util::getShopifyConfig('appbridge_version') ? '@'.config('shopify-app.appbridge_version') : '' }}"></script>
             <script src="{{config('shopify-app.appbridge_cdn_url') ?? 'https://unpkg.com'}}/@shopify/app-bridge-utils{{ \Osiset\ShopifyApp\Util::getShopifyConfig('appbridge_utils_version') ? '@'.config('shopify-app.appbridge_utils_version') : '' }}"></script>
        <script
            ...
            >
                ...
                var app = createApp({
                    ...
                    host: "{{ \Osiset\ShopifyApp\Util::getShopifyConfig('appbridge_host') ? config('shopify-app.appbridge_host') : '' }}",
                    forceRedirect: true,
                });
            </script>
